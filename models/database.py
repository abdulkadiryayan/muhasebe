...
class MuhasebeDB:
    ...
    def cari_hesap_ekle(self, musteri_adi, borc=0, alacak=0, islem_tarihi=None):
        self.cursor.execute('''
        INSERT INTO cari_hesap (musteri_adi, borc, alacak, islem_tarihi)
        VALUES (?, ?, ?, ?)
        ''', (musteri_adi, borc, alacak, islem_tarihi))
        self.conn.commit()

    def cari_hesap_guncelle(self, id, musteri_adi=None, borc=None, alacak=None):
        # Mevcut kaydı al
        self.cursor.execute('SELECT * FROM cari_hesap WHERE id=?', (id,))
        ...
        self.cursor.execute('''
        UPDATE cari_hesap 
        SET musteri_adi=?, borc=?, alacak=?, islem_tarihi=CURRENT_TIMESTAMP
        WHERE id=?
    ...
    def kasa_ekle(self, kasa_adi, gelir=0, gider=0, islem_tarihi=None):
        self.cursor.execute('''
        INSERT INTO kasa_yonetimi (kasa_adi, gelir, gider, islem_tarihi)
        VALUES (?, ?, ?, ?)
        ''', (kasa_adi, gelir, gider, islem_tarihi))
        self.conn.commit()

    def kasa_guncelle(self, id, kasa_adi=None, gelir=None, gider=None):
        # Mevcut kaydı al
        self.cursor.execute('SELECT * FROM kasa_yonetimi WHERE id=?', (id,))
        mevcut = self.cursor.fetchone()
        ...
        SET kasa_adi=?, gelir=?, gider=?, islem_tarihi=CURRENT_TIMESTAMP
        WHERE id=?
    ...
    def fatura_ekle(self, fatura_no, tutar, tur, islem_tarihi=None):
        self.cursor.execute('''
        INSERT INTO faturalar_irsaliyeler (fatura_no, tutar, tur, islem_tarihi)
        VALUES (?, ?, ?, ?)
        ''', (fatura_no, tutar, tur, islem_tarihi))
        self.conn.commit()

    def fatura_guncelle(self, id, fatura_no=None, tutar=None, tur=None):
        # Mevcut kaydı al
        self.cursor.execute('SELECT * FROM faturalar_irsaliyeler WHERE id=?', (id,))
        mevcut = self.cursor.fetchone()
        ...
        SET fatura_no=?, tutar=?, tur=?, islem_tarihi=CURRENT_TIMESTAMP
        WHERE id=?
